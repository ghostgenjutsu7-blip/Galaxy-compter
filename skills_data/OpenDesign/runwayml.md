---
name: Design System: runwayml
source: OpenDesign
version: 1.0.0
description: Category: AI & LLM
tags: ["design-system", "runwayml"]
triggers: ["runwayml"]
license: Apache-2.0
target_agent: design
 ...
 *   - --fg: #ffffff;
  ...
  --muted: #767d88 (Cool Slate);
  --meta: #7d848e (Mid Slate).
 *   - --border: #27272a (Border Dark);
  --border-soft: #1a1a1a (row
 *     separator that blends into the dark surface — "barely visible
 *     containment", per DESIGN.md §4).
 *   - --accent: #ffffff. Runway has no decorative accent color;
 ...
 *   - --radius-sm: 4px (sharp / button);
  --radius-md: 8px (card,
 *     image);
  --radius-lg: 16px (alert containers only — DESIGN.md §4).
...
:root {
  /* ─── Surface (3 levels) ──────────────────────────────────────────
   * Dark-dominant. The canvas is true black;
  --bg:           #000000;
  --surface:      #1a1a1a;
  --surface-warm: #030303;
  --fg:    #ffffff;
